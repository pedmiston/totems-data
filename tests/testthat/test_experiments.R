...
test_that("labeling player conditions duplicates guesses correctly", {
  guesses <- data_frame(
    Guess = 1,
    Result = 1,
    Strategy = c("Synchronic", "Diachronic", "Isolated", "Isolated"),
    PlayersPerSession = c(2, 1, 1, 1),
    SessionDuration = c(25, 25, 50, 25),
    Generation = c(1, 1, 1, 1),
    PlayerIX = 1,
    SessionIX = 1
  )
  results <- label_player_conditions(guesses)
  expect_equal(nrow(results), nrow(guesses) + 2)
  expect_equal(results$Exp, c("50LaborMinutes", "50LaborMinutes", "100LaborMinutes", "50LaborMinutes", "50LaborMinutes", "100LaborMinutes"))
})
